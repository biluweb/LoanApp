...
//font字体
gulp.task('fonts', () => {
  return gulp.src('src/Content/scss/mui-sass/fonts/*.ttf')
    .pipe(gulp.dest('dist/Content/css/mui-sass/fonts/'));
});
...
//代码格式化
gulp.task('pug', ()=> {
    return gulp.src('dist/Htmls/index.html')
    .pipe($.htmlBeautify({
        indent_size: 4,
        indent_char: ' ',
        // 这里是关键，可以让一个标签独占一行
...
//muijs
gulp.task('muijs', () => {
  return gulp.src('src/Content/scss/mui-sass/js/*.js')
    .pipe($.plumber())
//  .pipe($.concat('mui.js'))		//合并后的文件名
    .pipe(gulp.dest('dist/Content/css/mui-sass/js/'))
    .pipe(reload({stream: true}))
...
//layer
gulp.task('Contcopy',()=>{
    return gulp.src(['src/Content/**/*','!src/Content/scss/**/*'])
    .pipe($.plumber())
